...
pheno<-bd%>%select(f.eid, f.21003.0.0, f.31.0.0, f.189.0.0, f.54.0.0, f.22000.0.0)

colnames(pheno)<-c("IID", "Age", "Sex", "Townsend", "Assessment_center", "Geno_batch")

pheno2<-bd_join4%>%select(f.eid, f.21001.0.0, f.30690.0.0, f.30780.0.0, f.30760.0.0, f.30870.0.0)
...
new<-as_tibble(left_join(pheno, pheno2, by="IID"))
...
new$Sex<-mapvalues(as.character(new$Sex), c("Male", "Female"), c(0,1))
...
outdir="/scratch/as58810/FallProject"
...
#PCs
pan<-read_tsv("/scratch/as58810/FallProject/all_pops_non_eur_pruned_within_pop_pc_covs.tsv")
pan<-as_tibble(pan)
pan$s<-as.integer(pan$s)
table(pan$pop, useNA = "always")

bridge<-read.table("/scratch/as58810/FallProject/ukb48818bridge31063.txt")
...
#Output
outdir="/scratch/as58810/FallProject/pheno2"
